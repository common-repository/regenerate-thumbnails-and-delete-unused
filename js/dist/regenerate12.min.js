...
function($) {
    ...
    var data = {
        ...
        errorTpl = '<div class="regenerateErrorWrap"><a class="regenerateError1">' + wp_way2_regen_msgs.failed_h + '</a></div>',
        ...
        opts = '<option value="regenerate-bulk-lossy">' + wp_way2_regen_msgs.comp_all + '</option>';
    ...
    var getBulkImageData = function() {
        ...
        renderBulkImageSummary = function(bulkImageData) {
            var setting = regenerate_settings.api_lossy,
                nImages = bulkImageData.length,
                header = '<p class="regenerateBulkHeader"><strong>' + wp_way2_regen_msgs.bulkc + ' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </strong><span class="close-regenerate-bulk">&times;</span></p>',
                enjoyedEmAll = '<button class="regenerate_req_bulk">' + wp_way2_regen_msgs.comp_all + '</button>',
                typeRadios = '<div class="radiosWrap"><p>' + wp_way2_regen_msgs.auto_bulk_start_regen + '&nbsp;&nbsp;<label class="bulk-switcher2"><input type="checkbox" id="auto_regen_bulk_dummy2" name="_regenerate_options[auto_regen_bulk]" value="1"> <span class="slider round"></span><span class="select-y">Y</span><span class="select-n">N</span></label></p><label><input type="radio" id="regenerate-bulk-type-lossy" value="lossy" name="regenerate-bulk-type"/>' + wp_way2_regen_msgs.way2_lossy + '</label>&nbsp;&nbsp;&nbsp;<label><input type="radio" id="regenerate-bulk-type-lossless" value="lossless" name="regenerate-bulk-type"/>' + wp_way2_regen_msgs.loss_less + '</label>',
                $modal = $('<div id="regenerate-bulk-modal" class="regenerate-modal"></div>').html(header).append(typeRadios).append('<p class="the-following"><strong>' + nImages + '</strong> ' + wp_way2_regen_msgs.thumb_countss + ' ' + wp_way2_regen_msgs.opti_mess + ' </p><span class="regenerateBulkHeader1"></span><span class="figures_regen"><a id="rateus" title="' + wp_way2_regen_msgs.rt_us + '" href="https://wordpress.org/support/view/plugin-reviews/regenerate-thumbnails-and-delete-unused?rate=5#postform" target="_blank"><span class="likess2"></span></a></span><span class="bulkSetting" style="display:none">' + setting + "</span>").appendTo("body").kmodal(bulkModalOptions).bind($.kmodal.BEFORE_CLOSE, function(event, modal) {}).bind($.kmodal.OPEN, function(event, modal) {}).bind($.kmodal.CLOSE, function(event, modal) {
		...
		//	console.log(setting1);	
				$('#auto_regen_bulk_dummy2').on('change', function() {
            ...
            var $table = $('<table id="regenerate-bulk"></table>'),
                $headerRow = $('<tr class="regenerate-bulk-header"><td>' + wp_way2_regen_msgs.nameuu + ' <span class="buy2"><a href="https://way2enjoy.com/regenerate-thumbnails?pluginemail=' + admin_email_way2 + '" target="_blank"> ' + wp_way2_regen_msgs.buy_msg + '</a></span></td><td style="width:120px">' + wp_way2_regen_msgs.original_sz + '</td><td style="width:120px">' + wp_way2_regen_msgs.regenerate_st + '</td></tr>');
            $table.append($headerRow), $.each(bulkImageData, function(index, element) {
            ...
            }), $modal.append($table).append(enjoyedEmAll), $(".close-regenerate-bulk").click(function() {
                $.kmodal.close()
            }), nImages || $(".regenerate_req_bulk").attr("disabled", !0).css({
                opacity: .5
            })
        },
        bulkAction = function(bulkImageData) {
            $bulkTable = $("#regenerate-bulk");
            var parallelprocess = regenerate_settings.bulk_async_limit;
            if (parallelprocess === undefined) {
                var parallelp = '4';
            } else {
                var parallelp = parallelprocess;
            }
            var jqxhr = null,
                q = async.queue(function(task, callback) {
                    var id = task.id,
                        $row = (task.filename, $bulkTable.find("tr[data-regeneratebulkid='" + id + "']")),
                    ...
                    jqxhr = $.ajax({
                        ...
                        if (data.success && "undefined" == typeof data.message) {
                            var originalSize = (data.type, data.original_size),
                                ...
                                quotaRema = data.quota_remaining;
                            $regenerateedSizeColumn.html(data.html), $regenerateedSizeColumn.find(".regenerate-item-details").tipsy(tipsySettings) /*,$regenerateedSizeColumn.find(".regenerate-item-details").remove()*/ , $savingsPercentColumn.text(savingsPercent), $savingsBytesColumn.text(savingsBytes);
                            var $button = $("button[id='regenerateid-" + id + "']"),
                                $parent = $button.parent(),
                                $cell = $button.closest("td"),
                                $originalSizeColumn = $button.parent().prev("td.original_size"); 
								if(quotaRema===undefined){var ramukaka='Regeneration is 100% Free';}else{var ramukaka=''+quotaRema+'  ' + wp_way2_regen_msgs.quotabal_dtls + '';	}								
		$(".regenerateBulkHeader1").text(ramukaka);	
    ...
    $btnApplyBulkAction.add($btnApplyBulkAction2).click(function(e) {
        if ("regenerate-bulk-lossy" === $(this).prev("select").val()) {
            ...
            renderBulkImageSummary(bulkImageData), $(".regenerate_req_bulk").click(function(e) {
    ...
    $("body").on("click", ".regenerate-item-details", function(e) {
        ...
        var $t = $(e.target);
        $t.hasClass("tipsy") || $t.closest(".tipsy").length || $t.hasClass("regenerate-item-details") || (activeId = null, $(".regenerate-item-details").text(wp_way2_regen_msgs.shw_dtls), $(".tipsy").remove())
