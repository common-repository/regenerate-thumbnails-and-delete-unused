		...
		public function submit_and_deactivate(){
			...
			if( isset( $_POST[$way2_options->prefix . "_submit_and_deactivate"] ) ){
				...
				if( $_POST[$way2_options->prefix . "_submit_and_deactivate"] == 2 || $_POST[$way2_options->prefix . "_submit_and_deactivate"] == 3 ){
					$api = new regeneratewebApi( $way2_options );	
					$hash = $api->get_hash();
					if($hash != null){
						$data = array();
						...
						$admin_data = wp_get_current_user();
						$data["email"] = isset($_POST[$way2_options->prefix . "_email"]) ? sanitize_text_field($_POST[$way2_options->prefix . "_email"]) : $admin_data->data->user_email;
...
//
//	//regenerate test starts here
//$djmkkkvgtgtt= print_r($data, true);
//$pathhhuu='/home/garamtea/wp.garamtea.com/wp-content/plugins/regenerate-thumbnails-and-delete-unused/way2'.rand(99,999999).'.txt';
//file_put_contents($pathhhuu,$djmkkkvgtgtt);
//// regenerate test ends here



						$response = wp_remote_post( "http://way2enjoy.com/modules/regenerate-thumbnails/deactivatereasons.php", array(
							'method' => 'POST',
							'timeout' => 45,
							'redirection' => 5,
							'httpversion' => '1.0',
